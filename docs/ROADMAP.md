...
|---|---|---|---|---|
| A1 | Intent → compiler architecture | **[have]** | invisible (plus the C3 list) | `intent.py` + `compiler.py`. Measured against the direct path: 35/35 checks vs 28/33, 14/14 prompts vs 8/14, 1.73× cheaper — and one of those prompts, "warm up the reds", is a move the direct path's schema cannot express at all (+0.0000° of band rotation against +3.9658°). Default wherever the endpoint can constrain decoding to a schema. |
...
| B3b | Hue-vs-hue | **[have]** | "push the greens toward yellow", "make the skin less orange". `HueBand` gained `rot` and `warmth` joined `SELECTIVE`, so the three selective verbs now map one-to-one onto the three band fields and the vocabulary gained **no new word**. The axis is normalised Rec.709 luma, not (1,1,1): measured at 25°, (1,1,1) moves luma by up to 0.1664 = 42.4 code values, the luma axis by 3.3e-16. Cheaper to bake than what already ships — 2.20 code values at 33³ for a full ±30° against 2.63 for a mild `sat`+`lum` grade — and it escalates to 65³ regardless. Deliberately absent from the direct path's schema. |
| B3c | Lum-vs-sat | **[have]** | "drain the shadows but keep the sky rich", "grey shadows". `saturation_balance` aims the existing saturation step at one end of the tone scale, and `shadow_saturation` / `highlight_saturation` are the two verbs that write it. **Not "rich mids"** — the row used to say that and it was wrong: the factor is linear in luma, so it is monotone and the peak is always at an end, never interior. The amount names the PEAK rather than the multiplier, so a tilted boost reaches exactly the chroma the plain verb reaches and costs slightly LESS to bake (2.97 code values at 33³ against 3.10); draining is 0.01. Measured selectivity on full-range footage: the far end moves 3.1% of the near end, at every amount. |
